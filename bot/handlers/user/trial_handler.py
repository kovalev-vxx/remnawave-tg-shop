...
async def request_trial_confirmation_handler(
    ...
    confirmation_text = _("trial_confirmation_message")
    confirmation_keyboard = get_trial_confirmation_keyboard(current_lang, i18n)
    ...
    try:
        ...
        logging.warning(
            f"Could not edit trial confirmation message: {e_edit}. Sending new one."
        )
        if callback.message:
